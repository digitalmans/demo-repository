...
import com.hackathon.moodpixel.service.AgnesImageService;
...
public class PerlerCartoonController {
    ...
    public PerlerCartoonController(AgnesImageService agnesImageService) {
        this.agnesImageService = agnesImageService;
    }

    /**
     * 将上传的 JPEG/PNG 人像经 Agnes 图生图转为卡通风格，返回 base64 图片。
     */
    ...
    public CartoonizeResponse perlerCartoonize(@RequestPart("file") MultipartFile file) throws Exception {
        ...
        byte[] raw = agnesImageService.generateCartoonFromPortrait(file.getBytes(), ct);
        long t1 = System.nanoTime();
        log.info("[perler-cartoonize] Agnes 返回 bytes={} 耗时 {} ms", raw.length, (t1 - t0) / 1_000_000L);
