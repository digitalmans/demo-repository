...
public class TranslationService {
    ...
    @Value("${agnes.text-model:agnes-2.0-flash}")
    ...
    public String translateToChinese(String userText) {
        ...
        if (apiKey == null || apiKey.isBlank()) {
            log.warn("[Translate] 未配置 Agnes API Key，跳过翻译/润色，直接使用原文");
            return userText;
        }

        try {
            String url = combineUrl(apiBase, "/chat/completions");
            ...
            log.info("[Translate] 开始请求 Agnes 文本模型 model={} text={}", textModel, userText);
