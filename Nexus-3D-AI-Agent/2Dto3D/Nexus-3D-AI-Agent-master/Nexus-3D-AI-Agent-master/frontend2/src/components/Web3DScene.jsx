...
import { Environment, OrbitControls, useGLTF, useAnimations, GizmoHelper, GizmoViewport, Html } from '@react-three/drei';
...
// Avatar Component
// --------------------------------------------------------
function Avatar({ id, modelPath, position, animationTrigger, audioBase64, dynamicCode, destination, onReachDestination, manualOffset, manualRotation, avatarStatesRef, replyText, onAvatarChat, addLog }) {
  const group = useRef();
  
  const url = modelPath.startsWith('http') || modelPath.startsWith('file://') || modelPath.startsWith('blob:') || modelPath.startsWith('/')
      ...
      try {
        ...
        if (playPromise !== undefined) {
          playPromise.catch(error => {
            if (addLog) addLog(`[Audio Error] Cannot play voice: ${error.message}`);
          });
        }
        audio.onended = () => setIsPlayingAudio(false);
      } catch (err) {
        if (addLog) addLog(`[Audio Error] Setup failed: ${err.message}`);
      }
    } else if (animationTrigger === 'Talk') {
      if (addLog) addLog(`[Audio Warning] Backend returned no audio data for this speech.`);
    }
  }, [audioBase64, animationTrigger, addLog]);
      ...
      >
        <group ref={group} dispose={null}>
          <primitive 
            object={scene} 
            scale={[2.5, 2.5, 2.5]} 
            rotation={[0, (modelPath.includes('demo_robot.glb') || modelPath.includes('avatar_3d_')) ? -Math.PI / 2 : 0, 0]} 
          />
        </group>
      </group>
    </group>
...
// --------------------------------------------------------
export default function Web3DScene({ backgroundPath, avatars, showGrid = true, manualOffset, sceneOffset, manualRotation, sceneRotation, onReachDestination, avatarStatesRef, sceneObjectsRef, onAvatarChat, addLog }) {
  return (
