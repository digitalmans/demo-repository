...
def index():
    """主页"""
    return render_template('movie_qa.html')
...
def api_ask():
    ...
    try:
        ...
        # 获取答案
        retrieval_service = get_service()
        answer = retrieval_service.ask(question, top_k=1, threshold=0.1)
...
def api_search():
    ...
    try:
        ...
        # 搜索相关结果
        retrieval_service = get_service()
        results = retrieval_service.search(question, top_k=top_k, threshold=threshold)
...
def api_add_qa():
    ...
    try:
        ...
        retrieval_service = get_service()
        success, message = retrieval_service.add_qa(question, answer, category)
